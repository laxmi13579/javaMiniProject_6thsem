	...
	public void actionPerformed(ActionEvent e){
			    try {
			    	...
			    	if(password1.equals(confirm_password)) {
				    	...
				    	Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/mysql","root","mysql123");
				    	Statement stm = conn.createStatement();
				    
				    	PreparedStatement p = null;
				    	p = conn.prepareStatement("insert into details (name,phnos,email,dob,passwork) values(?,?,?,?,?)");
				    	p.setString(1,name1);
				    	...
				    	p.executeUpdate();
				    	JOptionPane.showInternalMessageDialog(null, "Resgistered:");
					    friend.main(null);
					    conn.close();
			    	}
			    	else {
